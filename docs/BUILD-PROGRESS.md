...
**UX follow-up (2026-06-29):** User feedback — the maintenance create flow opened the full tabbed detail (instant-create), unlike every other module, and Fleet/Maintenance sidebar items didn't match the other nav sections (no chevron, bespoke grey pill, duplicated map-pin icon). Fixed: (1) `/maintenance/new` is now a form-first create page (title, location, category, priority, description; Cancel / Create request) matching the enquiry pattern — the record is only created on submit and then opens on Overview; (2) Fleet and Maintenance are now full collapsible sidebar **sections** (own divider, chevron toggle, pink active state, expandable "All vehicles" / "All requests" sub-links) with dedicated truck / wrench icons — identical structure to Locations/Incidents.

**Verification (browser, localhost, signed in as Super User / AbilityVua Admin):** sidebar shows Fleet and Maintenance with chevrons matching every other section; Maintenance auto-expands on its route showing "All requests". Drove the create form end-to-end: filled title, switched location to Glenelg, set priority High, added a description, clicked Create request → created **MR-0004** "Broken window latch in lounge", redirected to its Overview tab (SLA computed 2026-06-30 04:00, audit footer + Full audit trail logged), and the register count went 3 → 4 with the new row. `npm run build` exit 0, `npx tsc --noEmit` exit 0, `npm run page-guides:check` exit 0. Bugbot: 1 medium (location picker async sync) fixed. Help article (maintenance) + TEST-102 step wording updated.
...
**Verification:** `npm run build` ✅ · `npx tsc --noEmit` ✅ · `npm run page-guides:check` ✅ (131 routes) · `npm run supabase:push-remote` ✅ (`20260728120000`) · localhost smoke ✅ end-to-end (importer apply → analyse 108 scanned / 82 impacts → consent-gated approve → apply 1; `price_update_run` persisted `applied`) · Amplify smoke ✅ (2026-06-28 — `/system/services/ndis-price-importer` + `/system/services/price-update-review`; analyse 108 scanned / 81 impacts; consent-required agreements; apply disabled without evidence)
...
## Glenelg SIL House — full roster of care demo data (2026-06-27)

**Status:** Generated, applied to remote Supabase, committed.

A realistic 24/7 NDIS SIL house roster of care for **Glenelg SIL House** (`loc-glenelg-sil`), generated by `scripts/generate-glenelg-roster-seed.mjs` → `supabase/seed-glenelg-roster.sql` (idempotent, additive).

| Area | Data |
|------|------|
| Residents | 3 — Bernadette Rose (existing) + new **Marcus Webb** (ID/autism, behaviours of concern) and **Priya Nair** (Down syndrome, epilepsy); home addresses at the house + resident links |
| Service agreements | NDIS SIL + Community Participation lines for each new resident (`sa-glen-marcus`, `sa-glen-priya`); Bernadette reuses `sa-rose-ni` |
| Staff | New **Jason Brown** support worker (Part-time, **30h**) + address, emergency contact, WWCC/NDIS screening/First Aid credentials; full Glenelg team assigned to the house (Noah, Amelia, Kai, Ivy Reed TL, Henry, Evie, casuals Chloe/Archie) |
| Roster of care | 65 master lines per the SIL pattern: **AM 07:00–15:00** (2 staff, 1:2), **PM 15:00–22:00** (2 staff, 1:2), **Sleepover 22:00–07:00** (1 staff, 1:3), plus a **Wed group day program 10:00–14:00** at the Adelaide Day Hub (Community Participation). Lines share `session_key` so they publish as one multi-client session |
| Staffing balance | Weekly rotation keeps full-timers ~37–39h, Jason ~31h, part-timers ~22h (logged as comments in the seed) |

**What you can test:** Open **Clients → Bernadette Rose / Marcus Webb / Priya Nair → Rostering** to see the master roster of care; open **Rostering** and publish a Glenelg week — matching `session_key` lines across the three residents merge into shared AM/PM/sleepover sessions with the rostered workers.

**Remote DB:** `npm run supabase:seed-glenelg` — exit 0 — `supabase/seed-glenelg-roster.sql` applied.

---

## Roster Sessions v1 — multi-client / multi-worker (2026-06-27)

**Status:** Built, verified, committed and pushed.

Introduces the session model: one live `roster_shift` block with multiple client lines (billing ratios) and worker lines (payroll), grouped from master RoC lines via `session_key`.

| Area | Change |
|------|--------|
| Schema | `20260702120000_roster_session_lines.sql` — `roster_shift_client_line`, `roster_shift_worker_line`, RoC line worker/ratio/session fields; backfill from legacy single client/worker |
| Data layer | Load/save session lines in `data-api.ts`; `normalizeRosterShift` syncs header from lines |
| RoC publish | `buildShiftsFromRosterOfCare` groups lines by session key + time/location; cross-RoC merge when same key |
| UI | Shift editor session participants panel + collective risk alerts; RoC card shows worker, ratio, session columns |
...
**Remote DB:** `npm run supabase:push-remote` — exit 0 — `20260702120000_roster_session_lines.sql` and `20260702123000_roster_rollover_settings.sql` applied.
...
**Migration history repair (2026-06-26):** Root cause of the desync was two pairs of local migrations sharing the same timestamp prefix (`20260623120000`, `20260625500000`), which made `supabase db push` try to insert a duplicate `schema_migrations` primary key. Fixed by renaming the second file in each pair to a unique version (`20260623130000_role_window_access_level`, `20260625505000_document_platform_support_plan`) and marking the 7 already-applied-but-untracked versions applied via `supabase migration repair --status applied`. All schema was already present on remote (verified via `information_schema`), so no data changed. Local and remote histories now match (125 = 125); `supabase db push` reports **"Remote database is up to date."**
...
|------|--------------|----------|--------|-------|
| 2026-06-29 | Support worker assistant — visible-client activity coach | 1 behaviour bug, fixed | **Pass** | When a support worker was already viewing a client record and clicked **Help me write today's activity update**, the backend only received `pagePath`; if route grounding did not resolve, the model asked for a client name and stopped at client lookup. Chat now sends the resolved visible client context (`id`, `name`, `searchKey`) to `/api/ai/chat`, and the coach always starts with **Step 1 — Confirm client** before loading notes. Regression covers the exact viewed-client flow; localhost browser smoke confirmed Step 1 → recent notes → activity-detail prompt → review / **Save activity** modal |
