...
| **Vehicle bookings** — reserve a vehicle for a driver, client, location, and time window | Admin / Rostering | Prevents double-booking and checks vehicle/driver compliance. Book from Fleet → Bookings, a maintenance request → Assignment, or a location → Vehicle bookings tab (location prefilled) | Live |
...
| **Security settings**; idle workspace timeout and inactivity warning | System operator | Unattended sessions expire after the organisation timeout and log as timed out | Live |
