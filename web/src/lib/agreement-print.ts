import {
  DEFAULT_AGREEMENT_TEMPLATE_ID,
  defaultAgreementTemplate,
  type DocumentTemplateRecord,
} from "@/lib/document-template";
import { renderDocument } from "@/lib/document-render";
import type { AgreementDocumentContext } from "@/lib/document-render-agreement";

export type AgreementPrintContext = AgreementDocumentContext;

export function buildAgreementPrintHtml(
  ctx: AgreementPrintContext,
  template: DocumentTemplateRecord = defaultAgreementTemplate()
): string {
  const result = renderDocument(template, ctx);
  if (result.blocked) {
    throw new Error(result.blocked);
  }
  return result.html;
}

export function printServiceAgreement(
  ctx: AgreementPrintContext,
  template?: DocumentTemplateRecord
): boolean {
  if (typeof window === "undefined") return false;
  try {
    const resolved = template ?? defaultAgreementTemplate();
    const result = renderDocument(resolved, ctx, { autoPrint: true });
    if (result.blocked || !result.html) return false;
    const win = window.open("", "_blank", "noopener,noreferrer");
    if (!win) return false;
    win.document.write(result.html);
    win.document.close();
    return true;
  } catch {
    return false;
  }
}

export function exportServiceAgreementHtml(
  ctx: AgreementPrintContext,
  template?: DocumentTemplateRecord
): { html: string; templateId: string } | null {
  const resolved = template ?? defaultAgreementTemplate();
  const result = renderDocument(resolved, ctx);
  if (result.blocked || !result.html) return null;
  return { html: result.html, templateId: resolved.id || DEFAULT_AGREEMENT_TEMPLATE_ID };
}
