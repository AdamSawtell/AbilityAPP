  ...
  sections: [
    {
      ...
      steps: [
        ...
        "Set module and tab access on each tile.",
        "Under a tab, tick Print or Send only when that role should generate documents from that tab.",
        "Grant workflow actions and reports as needed.",
        "Save the role.",
      ],
      relatedRoutes: ["/admin/roles"],
    },
    {
      id: "document-print-permissions",
      title: "Print and send permissions",
      body: "Document actions are separate from tab Write access. A role can edit a support plan but not print it, or print without send.\n\nPrint and send checkboxes appear under the relevant tab when the module is on. They only take effect when the tab is set to Write. Turning a tab to Read or Off clears document grants for that tab automatically.\n\nModule-level documents (for example Print invoice on Invoices) appear at the bottom of the module card and require Write on the module.",
      steps: [
      ...
      relatedRoutes: ["/admin/roles", "/clients"],
    },
    {
      id: "users",
